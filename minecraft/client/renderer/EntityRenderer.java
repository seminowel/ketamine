...
public class EntityRenderer implements IResourceManagerReloadListener {
   ...
   public void getMouseOver(float partialTicks) {
      ...
      if (entity != null && this.mc.theWorld != null) {
         ...
         Vec3 vec33 = null;
         float f = 1.0F;
         List list = this.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand((double)f, (double)f, (double)f), Predicates.and(EntitySelectors.NOT_SPECTATING, new Predicate() {
   ...
   private float getFOVModifier(float partialTicks, boolean p_78481_2_) {
      this.getClass();
      Entity entity = this.mc.getRenderViewEntity();
      GetFOVEvent event = new GetFOVEvent(p_78481_2_ ? this.mc.gameSettings.fovSetting : 70.0F);
   ...
   private void setupViewBobbing(float partialTicks) {
      if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
         ...
         GL11.glTranslatef(MathHelper.sin(f1 * 3.1415927F) * f2 * 0.5F, -Math.abs(MathHelper.cos(f1 * 3.1415927F) * f2), 0.0F);
   ...
   private void orientCamera(float partialTicks) {
      ...
      GetCameraPositionEvent cameraPosEvent = new GetCameraPositionEvent(entity.prevPosY + (entity.posY - entity.prevPosY) * (double)partialTicks);
      KetamineClient.getInstance().getEventBus().post(cameraPosEvent);
      double d1 = cameraPosEvent.getPos() + (double)f;
      double d2 = entity.prevPosZ + (entity.posZ - entity.prevPosZ) * (double)partialTicks;
      int thirdPersonMode = this.mc.gameSettings.getThirdPersonView();
      if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping()) {
         ...
         RayTraceCameraEvent rayTraceEvent = new RayTraceCameraEvent(4.0);
         ...
         if (!rayTraceEvent.isCancelled()) {
            double d4 = (double)(-MathHelper.sin(f1 / 180.0F * 3.1415927F) * MathHelper.cos(f2 / 180.0F * 3.1415927F)) * distance;
            double d5 = (double)(MathHelper.cos(f1 / 180.0F * 3.1415927F) * MathHelper.cos(f2 / 180.0F * 3.1415927F)) * distance;
            double d6 = (double)(-MathHelper.sin(f2 / 180.0F * 3.1415927F)) * distance;
   ...
   private void renderHand(float partialTicks) {
      ...
      boolean firstPerson = this.mc.gameSettings.getThirdPersonView() == 0;
   ...
   public void func_181560_a(float p_181560_1_, long p_181560_2_) {
      ...
      if (this.mc.inGameHasFocus && flag) {
         ...
         float f1 = f * f * f * 8.0F;
         UpdateLookEvent event = new UpdateLookEvent();
         KetamineClient.getInstance().getEventBus().post(event);
      ...
      i = this.mc.gameSettings.limitFramerate;
      if (this.mc.theWorld != null) {
         int j = Math.min(Minecraft.getDebugFPS(), i);
         j = Math.max(j, 60);
   ...
   private void renderWorldPass(ScaledResolution scaledResolution, float partialTicks, long finishTimeNano) {
      ...
      RenderManager render = this.mc.getRenderManager();
      double rx = render.getRenderPosX();
      double ry = render.getRenderPosY();
      double rz = render.getRenderPosZ();
               ...
               if (biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow()) {
                  ...
                  if (k2 != l2) {
                     ...
                     if (world.getWorldChunkManager().getTemperatureAtHeight(f2, j2) >= 0.15F) {
                        ...
                        d6 = this.random.nextDouble() + (double)f1 * 0.01 * (double)((float)this.random.nextGaussian());
                        d7 = this.random.nextDouble() + (double)(f1 * (float)this.random.nextGaussian()) * 0.001;
                        ...
                        int j4 = i4 >> 16 & '\uffff';
                        int k4 = i4 & '\uffff';
                        worldrenderer.pos((double)l1 - d3 + 0.5, (double)k2, (double)k1 - d4 + 0.5).tex(0.0 + d6, (double)k2 * 0.25 + d5 + d7).color(1.0F, 1.0F, 1.0F, f5).lightmap(j4, k4).endVertex();
                        worldrenderer.pos((double)l1 + d3 + 0.5, (double)k2, (double)k1 + d4 + 0.5).tex(1.0 + d6, (double)k2 * 0.25 + d5 + d7).color(1.0F, 1.0F, 1.0F, f5).lightmap(j4, k4).endVertex();
                        worldrenderer.pos((double)l1 + d3 + 0.5, (double)l2, (double)k1 + d4 + 0.5).tex(1.0 + d6, (double)l2 * 0.25 + d5 + d7).color(1.0F, 1.0F, 1.0F, f5).lightmap(j4, k4).endVertex();
                        worldrenderer.pos((double)l1 - d3 + 0.5, (double)l2, (double)k1 - d4 + 0.5).tex(0.0 + d6, (double)l2 * 0.25 + d5 + d7).color(1.0F, 1.0F, 1.0F, f5).lightmap(j4, k4).endVertex();
                     }
                  }
               }
            }
