...
public class DataWatcher {
   ...
   private WatchableObject getWatchedObject(int id) {
      ...
      try {
         datawatcher$watchableobject = (WatchableObject)this.watchedObjects.get(id);
   ...
   public static void writeWatchedListToPacketBuffer(List objectsList, PacketBuffer buffer) throws IOException {
      if (objectsList != null) {
         ...
         while(var2.hasNext()) {
            WatchableObject datawatcher$watchableobject = (WatchableObject)var2.next();
   ...
   public List getAllWatched() {
      ...
      for(Iterator var2 = this.watchedObjects.values().iterator(); var2.hasNext(); list.add(datawatcher$watchableobject)) {
         datawatcher$watchableobject = (WatchableObject)var2.next();
         if (list == null) {
            list = Lists.newArrayList();
         }
      }
   ...
   public void updateWatchedObjectsFromList(List p_75687_1_) {
      ...
      while(var2.hasNext()) {
         WatchableObject datawatcher$watchableobject = (WatchableObject)var2.next();
         WatchableObject datawatcher$watchableobject1 = (WatchableObject)this.watchedObjects.get(datawatcher$watchableobject.getDataValueId());
