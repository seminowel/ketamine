...
public class ChatUtil {
   ...
   public static String jsonToLegacy(String json) {
      if (json != null && !json.equals("null") && !json.isEmpty()) {
         try {
            String legacy;
            for(legacy = LegacyComponentSerializer.legacySection().serialize(ChatRewriter.HOVER_GSON_SERIALIZER.deserialize(json)); legacy.startsWith("§f"); legacy = legacy.substring(2)) {
            }
